<template>
  <div class="nav-menu">
    <div class="logo">
      <img class="img" src="~@/assets/img/logo.svg" alt="logo" />
      <span class="title" :v-if="!collapse">Vue3+TS</span>
    </div>
    <el-menu
      :default-active="currentMenuId"
      mode="vertical"
      class="el-menu-vertical"
      :collapse="collapse"
      background-color="#0c2135"
      :unique-opened="true"
      text-color="#b7bdc3"
      active-text-color="#0a60bd"
    >
      <template v-for="item in userMenus" :key="item.id">
        <!-- item.type===1 菜单有展开项 -->
        <templete v-if="item.type === 1">
          <el-sub-menu :index="item.id + ''">
            <template #title>
              <el-icon>
                <component :is="item.icon"></component>
              </el-icon>
              <span>{{ item.name }}</span>
            </template>
            <template v-for="subitem in item.children" :key="subitem.id">
              <el-menu-item
                :index="subitem.id + ''"
                @click="clickMenu(subitem.url)"
              >
                <template #title>
                  <el-icon>
                    <component :is="subitem.icon"></component>
                  </el-icon>
                  <span>{{ subitem.name }}</span>
                </template>
              </el-menu-item>
            </template>
          </el-sub-menu>
        </templete>
        <!-- item.type===2 菜单没有展开项 -->
        <templete v-else-if="item.type === 2">
          <el-menu-item :index="item.id + ''" @click="clickMenu(item.url)">
            <component :is="item.icon"></component>
            <template #title>
              <span>{{ item.name }}</span>
            </template>
          </el-menu-item>
        </templete>
      </template>
    </el-menu>
  </div>
</template>

<script lang="ts">
import { defineComponent, computed, ref } from 'vue'

import { pathToMenuId } from '@/utils/map-menus'

import { useStore } from '@/store'
import { useRouter, useRoute } from 'vue-router'

export default defineComponent({
  props: {
    collapse: {
      type: Boolean,
      default: false
    }
  },
  setup() {
    //store
    const store = useStore()
    const userMenus = computed(() => store.state.login.userMenus)

    //router
    const route = useRoute()
    const currentPath = route.path
    console.log(userMenus.value)
    console.log(currentPath)

    const currentMenu = pathToMenuId(userMenus.value, currentPath)
    console.log(currentMenu)

    const currentMenuId = ref(currentMenu.id + '')

    // 路由跳转
    const router = useRouter()
    const clickMenu = (url: string) => {
      console.log(url)

      router.push({
        path: url ?? '/not-found'
      })
    }
    return {
      userMenus,
      clickMenu,
      currentMenuId
    }
  }
})
</script>

<style scoped lang="less">
.nav-menu {
  height: 100%;
  width: 100%;
  background-color: #001529;

  .logo {
    display: flex;
    height: 28px;
    padding: 12px 10px 8px 10px;
    flex-direction: row;
    justify-content: flex-start;
    align-items: center;

    .img {
      height: 100%;
      margin: 0 10px;
    }

    .title {
      font-size: 16px;
      font-weight: 700;
      color: white;
    }
  }

  .el-menu {
    border-right: none;
  }

  // 目录
  .el-sub-menu {
    background-color: #001529 !important;
    // 二级菜单 ( 默认背景 )
    .el-menu-item {
      padding-left: 50px !important;
      background-color: #0c2135 !important;
    }
  }

  ::v-deep .el-sub-menu__title {
    background-color: #001529 !important;
  }

  // hover 高亮
  .el-menu-item:hover {
    color: #fff !important; // 菜单
  }

  .el-menu-item.is-active {
    color: #fff !important;
    background-color: #0a60bd !important;
  }
}
.el-sub-menu__title {
  width: 190px !important;
}

.el-menu-vertical:not(.el-menu--collapse) {
  width: 100%;

  height: calc(100% - 60px);
}
</style>
